                                        ...
                                        <div class="p-2">
                                            <h3 class="font-size-16">
                                                <strong>Daily Invoice Report
                                                    <span class="btn btn-info">{{ date('d-m-Y',strtotime($start_date)) }}</span> - <span class="btn btn-success">{{ date('d-m-Y',strtotime($end_date)) }}</span>
                                                </strong></h3>
                                        </div>
                                    </div>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        @foreach ($invoice as $key => $item)
                                                            <tr>
                                                                ...
                                                                <td class="text-center">{{ optional($item->payment)->customer->name }}</td>
                                                                <td class="text-center"># {{ $item->invoice_no }}</td>
                                                                <td class="text-center">
                                                                    {{ date('d-m-Y',strtotime($item->date)) }}</td>
                                                                <td class="text-center">{{ $item->description }}</td>
                                                                <td class="text-center">{{ optional($item->payment)->total_amount }}</td>

                                                            </tr>
                                                            @php
                                                                $total_price += optional($item->payment)->total_amount
                                                            @endphp
                                                        @endforeach
